...
export function FiguresList({ 
  ...
  const filteredFigures = figures.filter(figure => {
    ...
    const matchesVisibility = visibilityFilter === 'all' || figure.visibility === visibilityFilter
    
    const matchesDifficulty = difficultyFilter === 'all' || 
                             figure.metadata?.difficulty?.toString() === difficultyFilter
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        <Select value={visibilityFilter} onValueChange={setVisibilityFilter}>
          ...
          <SelectContent>
            ...
            <SelectItem value={FigureVisibility.WORKSPACE_SHARED}>Workspace</SelectItem>
      ...
      {filteredFigures.length === 0 ? (
        <div className="text-center py-12">
          ...
          <div className="text-gray-400 text-sm">
            {searchTerm || visibilityFilter !== 'all' || difficultyFilter !== 'all' 
        ...
        <div className="grid gap-4">
          {filteredFigures.map((figure) => (
            <Card key={figure.id} className="hover:shadow-md transition-all duration-200 cursor-pointer group">
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <CardTitle className="text-lg mb-2 truncate">{figure.name}</CardTitle>
                    <div className="flex items-center gap-2 mb-2">
                      <Badge className={`${getVisibilityColor(figure.visibility)} text-xs`}>
              ...
              >
                ...
                {figure.metadata?.tags && figure.metadata.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {figure.metadata.tags.slice(0, 3).map((tag, index) => (
                      <Badge key={index} variant="secondary" className="text-xs">
                        {tag}
                      </Badge>
                    ))}
                    {figure.metadata.tags.length > 3 && (
                      <Badge variant="secondary" className="text-xs">
                        +{figure.metadata.tags.length - 3} more
                      </Badge>
                    )}
                  </div>
                )}
